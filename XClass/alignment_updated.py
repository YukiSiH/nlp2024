...
def main():
    ...
    with open(os.path.join(data_dir, f"document_repr_updated.pk"), "rb") as f:
        dictionary = pk.load(f)
    ...
    if do_pca:
        ...
        if pca == 0:
            ...
            _pca.fit(document_representations)
            cumulative_explained_variance = np.cumsum(_pca.explained_variance_ratio_)
            # 设置累计解释方差达到90%的最小主成分数
            pca = np.argmax(cumulative_explained_variance >= 0.90) + 1
            print(f"Auto-determined PCA components: {pca}")
    ...
    with open(os.path.join(data_dir, f"data_alignment_updated.pk"), "wb") as f:
